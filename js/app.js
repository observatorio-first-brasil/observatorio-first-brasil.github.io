...
function viewTeam(arg) {
  ...
  // awards list (latest season)
  const aw = lastS.awards || [];
  app.append(card(
    h("h3", `Prêmios ${ys[ys.length - 1]}`),
    aw.length
...
function seasonTable(t, ys) {
  ...
  return el("div", { class: "tablewrap" }, el("table", {},
    el("thead", {}, el("tr", {},
      ...["Ano", "Score", IS_FTC ? "OPR perc." : "EPA n.", "Playoff", "Prêmios", "Ev.", ...(IS_FTC ? ["Nível alcançado"] : [])]
...
async function viewMethod() {
  ...
  app.append(card(
    ...
    el("p", {}, "Para cada equipe e temporada (2023–2026) combinamos quatro componentes, cada um normalizado em 0–1 e multiplicado pelo seu peso. O resultado é reescalado para 0–100."),
    el("div", { class: "chartbox sm" }, el("canvas", { id: "m-w" })),
  ));
  rankBars("m-w", COMP_KEYS.map((k) => ({
  ...
  add("Prêmios — peso " + pct(W.premios),
    `Cada prêmio conquistado vale conforme o <b>tier</b> de importância; a soma passa por retornos decrescentes (<code>1 − e^(−k·Σ)</code>). Prêmios de etapas mundiais são excluídos.` + tierTable(cfg.tier_weight || {}));
  add("Playoff — peso " + pct(W.playoff),
    `Quão longe a equipe foi no mata-mata (melhor resultado da temporada): Campeão ${scoreOf(cfg, "winner")}, Finalista ${scoreOf(cfg, "finalist")}, Semifinal ${scoreOf(cfg, "sf")}, rodadas iniciais ${scoreOf(cfg, "playoff")}. Bônus de <code>+${cfg.playoff_captain_bonus ?? 0.08}</code> para capitã de aliança.`);
  ...
  app.append(card(
    ...
    el("ul", {},
      ...
      el("li", {}, "Regionais fora do Brasil entram no cálculo. Etapas e prêmios do Mundial são excluídos. O EPA da temporada é a média dos EPAs dos eventos considerados."),
